...
pid_t Fork(void);//added wrapper function;
/*
...
struct job_t *job;
	...
	bg = parseline(cmdline, argv, &file);
	sigset_t mask;
	if(argv[0] == NULL)
		return;
	
	if(!builtin_cmd(argv)){
		//if it is not builtin_command
		sigemptyset(&mask);
